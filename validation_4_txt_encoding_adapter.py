...
from utils.utils_4_txt_encoding import dice_score, TEMPLATE,TEMPLATE_vein, ORGAN_NAME_VEIN, merge_label, visualize_label, get_key, NUM_CLASS
from utils.utils import extract_topk_largest_candidates, organ_post_process, threshold_organ,threshold_organ_4_txt_encoder,save_results
...
def validation(model, ValLoader, args, idx,snapshot_path,val_transforms = None):
    model.eval()
    dice_list = {}
    nsd_list = {}
...
def main():
    ...
    parser.add_argument('--result_save_path', default="/pub/data/yangdeq/CLIP/data/vein/out/used_for_pics", help='path for save result')
    ...
    snapshot_path = r"/pub/data/yangdeq/CLIP/data/vein/out/used_for_pics"
    store_path_root = f'/pub/data/yangdeq/CLIP/data/vein/model2024/0715/vein_semi_full_717_4classes_attention_512_concat_4txt_img_adapter/unet_epoch:1001_batch_size:3_0.0008/1/epoch_180_validation_DICE_AVG:0.7649_test_DICE_AVG:0.7612.pth'
    ...
    for store_path in glob.glob(store_path_root):
        # if "validation" not  in store_path and "320" in store_path:
            ...
            validation_loader, validation_transform = get_loader(args)
            if "DICE" not in store_path:
                i = int(store_path.split('_')[-1].split('.')[0])#+1 ### no rename 
                model_mean_dice = validation(model, validation_loader, args, i,validation_transform)
                new_name = store_path.replace(str(i), str(i)+"_" + args.phase +'_DICE_AVG:'+str(np.round(model_mean_dice,4)))
            else:
                i = 180#int(store_path.split('epoch_')[-1].split("_DICE")[0])
                # model_name = store_path.split('/')[-1].split('.')[0]
                model_mean_dice = validation(model, validation_loader, args, i,snapshot_path,validation_transform)

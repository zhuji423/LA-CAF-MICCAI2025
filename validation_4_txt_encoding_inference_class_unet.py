...
def validation(model, ValLoader, args, idx,snapshot_path,val_transforms):
    ...
    for index, batch in enumerate(tqdm(ValLoader)):
        ...
        for b in range(B):
            ...
            pred_save = torch.where(pred_sigmoid>0.5,1,0)
            # label_map = [1,2,1,2]
...
def main():
    ...
    model = unet_3D.unet_3D(n_classes=3, in_channels=1).cuda()

    unet_path = r"/pub/data/yangdeq/CLIP/comperation_results/unet3D-66.89/model1_iter_29600_test_dice_0.6689.pth"
    load_dict = torch.load(unet_path)
    model.load_state_dict(load_dict)
    print(f'Load {load_dict} weights')
    ...
    snapshot_path = r"/pub/data/yangdeq/CLIP/comperation_results/unet3D-66.89"

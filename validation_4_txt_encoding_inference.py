    ...
    for index, batch in enumerate(tqdm(ValLoader)):
        ...
        for b in range(B):
            ...
            for i in range(1,pred_save.shape[1]+1):
                pred_save[0, i-1, pred_save[0, i-1] > 0] = label_map[i-1] ## save for 2 class
            # for i in range(1,pred_save.shape[1]+1):
            #     pred_save[0, i-1, pred_save[0, i-1] > 0] = i  ## save for 4 class
            ...
            save_results_4_veins(batch, save_path, val_transforms, organ_list)
        torch.cuda.empty_cache()
...
def main():
    ...
    ## dataset
    parser.add_argument('--dataset_list', nargs='+', default=['vein_semi_full_']) # 'vein_artery' 'vein_semi_full_'
    parser.add_argument('--data_root_path', default='/pub/data/yangdeq/CLIP/data/vein/', help='data root path')
    parser.add_argument('--data_txt_path', default='/home/yangdq/project/module/CLIP-Driven-Universal-Model/dataset/vein_semi_full_list/', help='data txt path') #vein_semi_full_list vein_list
    parser.add_argument('--train_txt', nargs='+', default="train.txt")#_train_hessian_full.txt   'train.txt'
    parser.add_argument('--val_txt', nargs='+', default='val.txt')  #_val_hessian_full.txt  val.txt
    parser.add_argument('--test_txt', nargs='+', default='test.txt') #_test_hessian_full.txt  test.txt
    ...
    # store_path_root = r"/pub/data/yangdeq/CLIP/data/vein/model2024/0715/vein_semi_full_717_4classes_attention_512_concat_4txt_img_adapter/unet_epoch:1001_batch_size:3_0.0008/1/epoch_20_validation_DICE_AVG:0.6393_test_DICE_AVG:0.6285.pth"
    ...
    for store_path in glob.glob(store_path_root):
        # if "400" not in store_path:
            ...
            snapshot_path = r"/pub/data/yangdeq/CLIP/comperation_results/ours"

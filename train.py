...
def process(args,snapshot_path):
    ...
    if args.pretrain is not None:
        ...
        for key, value in load_dict.items():
            ...
            num_count += 1
        ## 新加的两个类别 使用线性投射层
        zeros_tensor = torch.zeros([2, 512]).to(args.device)
...
def main():
    ...
    ## logging
    parser.add_argument('--log_name', default='model2024/vein_full_train_CLIP_univer_34_class_val_test_N700_300_hessian', help='The path resume from checkpoint')
    ## model load
    ...
    parser.add_argument('--pretrain', default="/pub/data/yangdeq/CLIP/pretrained_weight/unet.pth",  
                        help='The path of pretrain model. Eg,/pub/data/yangdeq/CLIP/pretrained_weight/swinunetr.pth ')
    parser.add_argument('--trans_encoding', default='word_embedding', 
                        help='the type of encoding: rand_embedding or word_embedding')
    parser.add_argument('--word_embedding', default='/pub/data/yangdeq/CLIP/pretrained_weight/txt_encoding_artery_vein.pth', 
    ...
    ## hyperparameter
    parser.add_argument('--max_epoch', default=801, type=int, help='Number of training epoches')
    ...
    parser.add_argument('--lr', default=6e-4, type=float, help='Learning rate') ## 1e-4 
    ...
    parser.add_argument('--num_workers', default=10, type=int, help='workers numebr for DataLoader')
    ...
    parser.add_argument('--roi_x', default=96, type=int, help='roi size in x direction') #96
    parser.add_argument('--roi_y', default=96, type=int, help='roi size in y direction')#96
    parser.add_argument('--roi_z', default=96, type=int, help='roi size in z direction')#96
    ...
    parser.add_argument('--cache_rate', default=0.7, type=float, help='The percentage of cached data in total')

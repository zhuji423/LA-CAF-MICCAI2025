...
from utils.utils import dice_score, threshold_organ, visualize_label, merge_label, get_key,threshold_organ_4_txt_encoder,organ_post_process
from utils.utils_4_txt_encoding import TEMPLATE_vein, ORGAN_NAME, NUM_CLASS,save_results_4_veins
...
def validation(model, ValLoader, val_transforms, args):
    ...
    for index, batch in enumerate(tqdm(ValLoader)):
        ...
        organ_list = [i for i in range(1,5)]  ###using for flare2024
        # organ_list = [26, 32]
        # if 'liver' in name[0]:
        #     organ_list = [6, 27]
        ...
        pred_hard_post = torch.tensor(pred_hard)
        batch['results'] = pred_hard_post

        save_results_4_veins(batch, args.result_save_path, val_transforms, organ_list)
...
def main():
    ...
    ## dataset
    parser.add_argument('--data_root_path', default="/pub/data/yangdeq/CLIP/data/vein/25_vein_artery/semi_full_labelsTr_ori/", help='data root path')
    ...
    checkpoint = torch.load(store_path_root)
    ...
    print('Use pretrained weights. load', 'params into', len(store_dict.keys()))

...
class Universal_model_adapter(nn.Module):
    def __init__(self, img_size, in_channels, out_channels, backbone = 'swinunetr', encoding = 'rand_embedding',is_concat = True):
        ...
        for cls in range(out_channels):
            adapter = Adapter(512,256 ,skip_connect = False)  
        ...
        self.imgAdapter =  Adapter(512,512,skip_connect = False)  # with skip connectiona pply for img embedding 
        #### ADDING ADAPTER LAYERS according to number of classes
        if backbone == 'unet':
            ...
            self.Feature_adapter2 = nn.Sequential(
                Adapter(12,1,skip_connect = False),
            )
            self.Feature_adapter3 = nn.Sequential(
                nn.Conv3d(512, 256, kernel_size=1, stride=1, padding=0)
            )
        else:
    ...
    def forward(self, x_in):
        ...
        if self.encoding == 'rand_embedding':
            ...
            for idx, blk in enumerate(self.txtAdapter):
                ...
                # print(idx,blk(self.organ_embedding[idx]).max())
        ...
        x_feat = self.Feature_adapter1(dec4)
        new_x_feat = []
        x_feat_squeeze = []
        for batch in range(x_feat.shape[0]):
            x_feat_squeeze = x_feat[batch]
            for i in range(3):               
                x_feat_squeeze = self.Feature_adapter2(x_feat_squeeze)
                # print("before",x_feat_squeeze.shape)
                x_feat_squeeze = x_feat_squeeze.squeeze(-1)
                # print(f"x_feat_squeeze.max:{x_feat_squeeze.max()},x_feat_squeeze.min:{x_feat_squeeze.min()}")
            new_x_feat.append(x_feat_squeeze)
        x_feat = F.relu(torch.stack(new_x_feat))
        # x_feat =torch.stack(new_x_feat)
        x_feat = x_feat.unsqueeze(2).unsqueeze(2).unsqueeze(2) # task_encoding torch.Size([4, 512, 1, 1, 1])
        x_feat = self.Feature_adapter3(x_feat)
        ### use mlp+conv to replace Global average pooling + conv

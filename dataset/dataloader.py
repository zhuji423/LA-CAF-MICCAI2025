...
def get_loader(args):
    ...
    for item in args.dataset_list:
        for line in open(args.data_txt_path + item + args.train_txt):
    ...
    val_img_name = []
    for item in args.dataset_list:
        for line in open(args.data_txt_path + item + args.val_txt):
            name = line.strip().split()[1].split('.nii.gz')[0]
            img_name = line.strip().split()[0].split('.nii.gz')[0]
            ...
            val_img_name.append(img_name)
    data_dicts_val = [{'image': image, 'label': label, 'post_label': post_label, 'name': name,'img_name': img_name}
                for image, label, post_label, name,img_name in zip(val_img, val_lbl, val_post_lbl, val_name, val_img_name)]
    ...
    test_img_name = []
    for item in args.dataset_list:
        for line in open(args.data_txt_path + item + args.test_txt):
            name = line.strip().split()[1].split('.nii.gz')[0]
            img_name = line.strip().split()[0].split('.nii.gz')[0]
            ...
            test_img_name.append(img_name)
    data_dicts_test = [{'image': image, 'label': label, 'post_label': post_label, 'name': name, 'img_name': img_name}
                for image, label, post_label, name, img_name in zip(test_img, test_lbl, test_post_lbl, test_name, test_img_name)]
    ...
    if args.phase == 'train':
        ...
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None), num_workers=args.num_workers, 
                                    collate_fn=list_data_collate, sampler=train_sampler,pin_memory = True)
    ...
    if args.phase == 'validation':
        ...
        val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=4, collate_fn=list_data_collate,pin_memory = True)
    ...
    if args.phase == 'test':
        ...
        test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=4, collate_fn=list_data_collate,pin_memory = True)
